...
// Depth 3 with quiescence, and two ideas: `kingDanger` at -40, which reads what attacks the squares
...
//
// `kingDanger` alone sat barely above bare material on that search (+19 to +100 across runs). Adding `development`
// put it ~+110 on top and near the middle of the gap below the Tiger. `centerControl` as a
...
// openings against bare material at depth 3 it cut queen moves made with two minors still home from 34 to 10. At
...
// within noise both times — the gap below the Tiger was flat before this, so a nudge that never
