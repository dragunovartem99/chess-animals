...
| Commit                                                      | Contents                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Green when                                                               |
| ----------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| ⬜ `eval: one extractor per feature in c`                   | The C families mirror the TS files, not the work: they share only the lazy attack walk, which the context already caches, and a family runs whole for one weighted feature (`aggression` walks for `pushDepth`, which needs no walk). One function per feature, a table from id to function, and the evaluator picks the bot's extractors beside its live slots, so a node runs exactly what the bot weighs, with no guard on a weight. The bench reports ns per feature; CLAUDE.md's "one line in its family's extractor" becomes "one C function" | Bench signature unchanged; material-only search ≥ 5× faster in wasm      |
| ⬜ `engine: ask a leaf for any legal move, not all of them` | A leaf generates the whole move list only to tell mate and stalemate from a position that plays on — about 130 ns of a ~200 ns leaf once extraction is per feature. `has_legal_move` stops at the first legal move it finds, king moves first since they are the likeliest to exist and cheapest to prove; without quiescence the leaf needs nothing more. With quiescence the list is still used for the captures, so there it pairs with staged generation below                                                                                  | Games identical, bench signature unchanged, Mnps reported in the message |
