...
- ✅ `tune: reuse one worker pool across the SPSA run` — `cli/tune.ts` handed `runTuning` a bare `runGames`, which builds and tears down `availableParallelism()` workers per call — ~80 times over a default run, each repaying the tsx loader startup. It now shares one `createGamePool` like `cli/arena.ts` and closes it at the end
- ✅ `frankenstein: handshake the sandbox worker once` — `buildEngine` returned the raw client, so `stepOnce`'s `await engine.init()` paid the `uci`/`isready` round trips every autoplay move. It now memoises the handshake promise like `useBotEngines`. The stale comment claiming a local in-thread engine went with it
- ✅ `frankenstein: mutate the sandbox weights in place` — `setWeight` spread a fresh ~30-key record per slider input tick, re-rendering every other slider. The ref is deep-reactive, so a single-key assignment is enough
- ✅ `board: drop the deep flag on the chessground watch` — the watched `config` is a computed that returns a fresh object per recompute, so the shallow watch already fires on every change; `deep` only bought a full traversal of the config (the dests Map included) per move
- ✅ `scheduler: one seed mixer and one pair key` — the tuner's `seedFrom` and the scheduler's `mixSeed` were byte-identical FNV-1a (a reproducibility trap with two copies), and `pairKey`/`pairKeyOf` were the same function twice. One `mixSeed` in `scheduler/seed.ts`, one ternary `pairKey` in `pairing.ts`; the doubled `pairKey(a, b)` calls in `adaptiveRating` and `tournament` are hoisted
- ✅ `game: send the play view's engines the move history` — `PlayView` asked for a move with `{ fen }` alone, so the worker had no history to rebuild the repetition table from and a temperature-0 bot-vs-bot game could only end at the 300-ply cap. It now passes the move list
- ✅ `engine: let the check state skip the mate probe in quiescence` — every quiescence node paid `isCheck()` and then, inside the evaluation, `terminalScore`'s `isCheckmate()` (each recomputes the checkers, and `isCheckmate` also scans for a legal move). Quiescence now computes `isCheck()` once and threads it to `terminalScore` as a hint: a known check state answers the mate and stalemate probes that could only come back false. Scores are unchanged — `isCheckmate` needs a check and `isStalemate` needs its absence — and `search.bench.ts` shows no regression
- ✅ `engine: don't re-test a draw negamax already ruled out` — the quiescence entry re-ran `drawn(position)` on the exact position negamax had just tested before descending to the leaf. `quiesce` (with the test) now wraps `quiesceNode` (without), and only the entry from the leaf skips it — the recursion still tests every position it makes. A negamax node with no legal moves likewise hands its check state to the evaluation. Scores unchanged, bench flat
- ✅ `board: stop re-parsing the FEN in ChessBoard` — dropped an unused `parseSquare` import and derived the promotion picker's colour from `config.turnColor` instead of a second `positionFromFen` of the same string
- ✅ `scheduler: hash a game key once per spec` — a cache miss canonicalised and SHA-256'd the spec twice, once for the `get` and again for the `set` that followed. A `WeakMap` memo keyed by the spec object collapses it to one
- ✅ `tuner: track the SPSA best with two plain comparisons` — the best-probe update built a tuple-of-tuples and looped it to save writing two `if`s; the two `if`s are shorter and allocate nothing
- ✅ `engine: delta-prune hopeless captures in quiescence` — every capture was searched, however far behind the position already was. One that could not lift the standing score to `alpha` with the piece thrown in and two pawns on top is skipped now, priced from the bot's own material and `captureValue` weights rather than assumed values — a bot that counts no material gets a bound of zero, which is the truth about what a capture can do to its score. ~13% at depth 3 + quiescence taking the argmax and ~16% sampling, against the same best moves on the tactical fixtures
- ✅ `engine: score repetition, the fifty-move rule and insufficient material` — the search knew about mate and stalemate and nothing else, so a bot a queen up would walk back into a position the game had already stood in twice and score it as winning, and none of the shufflers could tell they were shuffling. A drawn position now scores a flat zero before it is evaluated and before its moves are generated. The history reaches the search as a stack of 64-bit position hashes — `repetitionKey`'s exact FEN is the game-level rule's and the openings', and a string per node is what a search cannot afford — consulted only once `halfmoves` says a repetition is reachable. Costs 2–4% on the spread, and ~6% at depth 1, where a node does least
- ✅ `eval: share one classical piece-value table` — move ordering, `captureValue` and `offeredMaterial` each carried an identical `{ pawn: 1, …, queen: 9 }` literal with its own copy of the same paragraph explaining why it must not be the tunable material weights. One `CLASSICAL_VALUES` in `shared/chess` says it once, and living below `eval` is what makes it structurally unable to become them
- ✅ `perf: gate a family's expensive features on the weights that read them` — `createExtractor` already skipped a family a bot leaves at zero, but a family is not the right grain: `reverseStarting` walks both armies against every home square and each symmetry walks the board again, so the Wolf, which names `swarm` alone, spent more of its search on the three proximity features it does not weigh than on the one it does. `EvalContext.weighs` asks the question per feature and the four board-walking features are held behind it; `reachable.test.ts` holds every feature to reading the same value asked for alone as in the full vector. Wolf ~2.4×, Shark ~2.9×, Turtle ~2.2×, Snake ~1.7× over a spread of positions
- ✅ `perf: reuse one position per ply instead of cloning one per move` — the search descends a single line at a time, so a ply's position is dead the moment its subtree returns; `createDescend` keeps one per ply and overwrites it, in place of the fresh `Chess` — a board and six little castling objects — that `afterMove` allocated and threw away for every move it tried. Roughly a quarter of the search was in that clone and the collector behind it
- ✅ `perf: generate moves without the generator, and skip the legality work a capture list cannot use` — a `SquareSet`'s `for..of` is a generator, an iterator object and a `{ value, done }` per square, in what is the hot loop of every search; the move generator walks the bits instead. `legalCaptures` now asks first whether a piece attacks anything worth taking, which is far cheaper than the pins-and-checks work `dests` does for a piece with no capture to make, and move ordering sorts the list it was handed rather than a copy of it, testing occupancy before `getRole` scans six role sets to report an empty square. Material counting drops the ten `SquareSet`s a node it allocated only to measure. Depth 3 ~1.7×, depth 2 ~1.6×, depth 3 + quiescence ~1.6×, with every bot's scores bit-identical
...
- ✅ `engine: break a root tie by the search order, not by the scores` — the epsilon window below bought correctness with every tie's cutoff, at depth 2 770 → 490 hz and depth 3 + quiescence 18.6 → 1.9. The scores were never the place to look: a pruned search reports a bound, so `searchRoot` returns the move it settled on and the argmax reads that, which only a move that strictly beat every move before it can be. The root's order is then the tie-break, so an rng shuffles it — and the root is _not_ sorted on top of the shuffle, which was the first attempt: `orderMoves` puts captures first, so every tie went to the same move and the Donkey, whose moves all tie, took every capture on the board and scored 73% against the Shark instead of 27%. Deeper nodes still order, which is where it pays. Back to 797 hz at depth 2, 125 at depth 3 and 19.8 with quiescence, all of it above where the bug had it
- ✅ `engine: keep a root cutoff off the window` — searching a root move against exactly `-best` cuts off when its score merely _reaches_ the best, and a cutoff reports the bound, so a strictly worse move came back reading exactly `best`. The argmax's tie-break cannot tell a bound from a value and spread over it: the Parrot answered 1.e4 e5 2.Nf3 with 2...Ne7 one game in three, scoring it 500 points below 2...Nf6, and every argmax bot in the roster had the same hole — the guard held pruning to keeping the real ties and never to dropping the false ones. One epsilon of window is the fix. It costs the speed the bug was quietly buying: depth 2 770 → 490 hz, depth 3 138 → 87, depth 3 + quiescence 18.6 → 1.9, since a tie no longer cuts off at all. The whole roster is re-rated on it
- ✅ `bots: add the Parrot, a copycat` — `symmetryMirrorY` was in the registry with no bot reading it; on the `material` base at 150 it mirrors a move until mirroring would cost a knight. The arena rates it 1524, between the Shark and the Snake. Its depth has to be even: a symmetry reads the same from either seat, so negamax flips its sign every ply and the depth-1 Parrot answers 1.e4 with 1...a5
