...
// The C engine's board is checked against chessops on this corpus: each line is a position, one
...
//
// `npm run engine:corpus` rewrites the fixture; the seed makes it byte-identical run to run.
const GAMES = 1000;
const MAX_PLIES = 120;
// One ply in SAMPLE is kept at random, so the start position does not repeat once per game.
const SAMPLE = 100;
const OUTPUT = new URL("../engine/tests/fixtures/moves.txt", import.meta.url);
...
console.log(`${lines.length} lines -> ${OUTPUT.pathname}`);
