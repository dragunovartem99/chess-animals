...
// roster. The file opens with one `bot` line per animal — its id and its compiled weights as
// float32 bits — then one line per sample: its head, and each animal's score in roster order as
...
export function evalLines(): string[] {
	...
	return [
		...bots.map((bot) => `bot;${bot.id};${bits32(bot.weights)}`),
		...samples.map((sample) => {
